...
class Task:
    ...
    def task_to_dict(self):
        # Konvertiert die Task-Instanz in ein Dictionary zur einfachen Speicherung
        return {
            'title': self.title,
    ...
    def dict_to_task(data):
        # Erstellt eine Task-Instanz aus einem Dictionary
        title = data['title']
    ...
    def save_tasks_to_file(tasks, filename):
        # Speichert eine Liste von Aufgaben in einer JSON-Datei
        with open(filename, 'w') as f:
            json_tasks = [task.task_to_dict() for task in tasks]
            json.dump(json_tasks, f, indent=4)

    def load_tasks_from_json(filename="tasks.json"):
        # Lädt Aufgaben aus einer JSON-Datei und gibt eine Liste von Task-Objekten zurück
        try:
            with open(filename, 'r', encoding='utf-8') as f:
                task_dicts = json.load(f)
                # 1. Deserialisierung: Konvertiere alle Dictionaries zurück in Task-Objekte
                tasks = [dict_to_task(d) for d in task_dicts]
                return tasks
        except FileNotFoundError:
            print("No tasks.json file found. Starting with empty list.")
            return []
